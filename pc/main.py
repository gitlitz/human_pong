...
class SerialWatcher(object):
    def __init__(self, serial_dev, default_value=100):
        super(SerialWatcher, self).__init__()
        self.serial_dev = serial_dev
        self._val = multiprocessing.Value('d', default_value)
    ...
    def _run(self):
        while True:
            txt = self.serial_dev.readline()
            logging.info("new pos: " + repr(txt))
            with self._val:
                try:
                    val = float(txt)
                    if val < 10:
                    ...
                    logging.warning("failed to parse " + repr(txt))
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, y_pos, x_serial_dev):
        ...
        self.rect.x = 0
        self.rect.y = y_pos
        self.x_serial_dev = x_serial_dev
    ...
    def update(self):
        ...
        try:
            print(repr(self.x_serial_dev.get()))
            self.rect.x = int(self.x_serial_dev.get() * 4)
        except ValueError:
            pass
...
def main():
    ...
    player1_serial = SerialWatcher(serial.Serial("/dev/ttyUSB0", baudrate=9600))

    # Create the player paddle object
    player1 = Player(580, player1_serial)
    player2 = Player(25, player1_serial)
    ...
    pygame.quit()  # --- Import libraries used for this program
